...
- **http_request** — `POST` JSON na zmail (`https://hub.ag3nts.org/api/zmail` lub `ZMAIL_API_URL`). Pole `apikey` w body ustawiają narzędzia zmail / env — **nie wymyślaj**. Użyj do **`help`**, **`getInbox`**, **`getThread`** (kształt body jak w API kursu).

...
export async function loadMcpConfig(rootDir: string): Promise<McpConfig> {
  const raw = await readFile(resolve(rootDir, '.mcp.json'), 'utf-8').catch(
    () => null,
  );
...
function createStdioTransport(
  serverConfig: McpServerConfig & { transport?: 'stdio' },
  rootDir: string,
) {
  ...
  async function callTool(
    ...
    if (!client) {
      if (authRequired.has(parsed.server)) {
        throw new Error(
          `MCP server "${parsed.server}" requires OAuth authorization`,
        );
      }
